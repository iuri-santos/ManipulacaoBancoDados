---
title: "Desafio - 05"
author: "Iuri Santos Oliviera 194610"
format: 
  html:
    self-contained: true
editor: visual
...
```
1. Extraia da tabela albums todas as colunas e todas as linhas e armazene-os
em um objeto chamado album_db
```{r}
album_db = dbGetQuery(db, 'SELECT * FROM albums')
head(album_db)
```

2. Selecione as colunas trackid, name, composer e unitprice. Ordene o objeto
...
```
5. Limitando-se a 5 registros, após ordenação por nome, quais são os nomes, ID
...
6. Quais são as músicas e identificadores de faixa e álbum produzidas pelo
...
dbGetQuery(db, sql)
```
7. Quais são as faixas cujos nomes começam com qualquer caracter seguido de
