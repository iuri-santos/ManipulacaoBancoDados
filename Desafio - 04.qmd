...
#baixando todos os pacotes necessC!rios para a realizaC'C#o desse cC3digo.
...
c.  Produza um mapa que apresente todo o trajeto voado pela aeronave ao longo de todo o ano; o trajeto deve ser apresentado de maneira linear no tempo (i.e., segue a sequC\*ncia do tempo, como no exemplo hipotC)tico dado acima);

d.  O mapa deve ser decorado com estatC-sticas do seu interesse (por exemplo, velocidade mC)dia do vC4o como espessura da linha que conecta os aeroportos envolvidos no trajeto);

## Parte 2:

a.  Seu cC3digo deve ser bem comentado;

b.  Os nomes das variC!veis devem ser esclarecedores (por exemplo, x C) um nome ruim);

c.  Seu cC3digo serC! avaliado por 5 de seus colegas;

d.  A parte de avaliaC'C#o (descrita em c.) acontecerC! na semana de 23 a 27 de setembro (com entrega obrigatC3ria no dia 27);

## ObservaC'C5es:

1.  A funC'C#o terC! o argumento tail_number e arquivo;

2.  O argumento tail_number serC! o cC3digo identificador do aviC#o de interesse (por ex., N431WN);

3.  O argumento arquivo serC! o nome do arquivo com caminho completo (por ex., file.path("dados", "flights.csv.zip"));

4.  A funC'C#o se chamarC! analisa_aeronave;
...
7.  A funC'C#o deve retornar uma lista (nomeada) com dois valores: a tabela utilizada para o grC!fico e o grC!fico propriamente dito;

8.  O Jupyter nC#o serC! suficiente para a realizaC'C#o da tarefa; utilize o RStudio; o laboratC3rio continuarC! disponC-vel nos horC!rios da aula;

9.  O cC3digo deverC! ser submetido via Moodle atC) o dia 23 de setembro, C s 10 da manhC# (em C!rea exclusiva a ser disponibilizada oportunamente).

## CC3digo:

```{r}
#Criando a funC'C#o para a realizaC'C#o do cC3digo
analisa_aeronave <- function(tail_number, arquivo) {
  ...
  # Criando uma funC'C#o chamada callback
  ...
  trajeto_filtrado <- flights %>%
    mutate(
      ...
      # Criando variC!vel de data a partir de ano/mC*s/dia
      DATA = make_date(YEAR, MONTH, DAY),
      # Data formatada no padrC#o brasileiro DD/MM/YYYY
      DATA_FORMATADA = format(DATA, "%d/%m/%Y"),
      # Dia da semana do voo (segunda, terC'a, etc.)
  ...
  trajeto_coord <- trajeto_filtrado %>%
    ...
    mutate(
      # Velocidade mC)dia do voo (milhas por hora)
    ...
    # Calculando dia da semana mais frequente de saC-da de cada aeroporto
  ...
  for (i in 1:nrow(trajeto_coord)) {
    mapa_base <- mapa_base %>%
      addPolylines(
        ...
        # Espessura da linha cresce com a distC"ncia do voo 
        ...
        popup = paste0("De ", trajeto_coord$ORIGIN_AIRPORT[i], 
                       ...
                       "<br><b>DistC"ncia:</b> ", trajeto_coord$DISTANCE[i], " milhas",
                       "<br><b>Velocidade mC)dia:</b> ", round(trajeto_coord$velocidade_media[i], 1), " mph")
      )
  }
  
   # Criando tabela selecionando as colunas de interesse e removendo a repetiC'C#o de aeroportos
  ...
  mapa_base <- mapa_base %>%
    addCircleMarkers(
      ...
      popup = paste0("Aeroporto: ", aeroportos_unicos$ORIGIN_AIRPORT,
                     "<br>Maior frequC*ncia: ", aeroportos_unicos$dia_frequente)
    )
  ...
  # Retornando uma variC!vel contendo tabela + mapa
  return(list(
...
# Input (basta escolher a aeronave a ser analisada(TAIL_NUMBER) e mudar o caminho para o dado, caso necessC!rio)
